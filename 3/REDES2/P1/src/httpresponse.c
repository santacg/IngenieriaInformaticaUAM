...
void methodGet(int connfd, char *path, int num_headers, int minor_version);

void methodPost(int connfd, char *path, int num_headers);
...
void methodPost(int connfd, char *path, int num_headers) { 
  ...
  resource_fd = open(path, O_CLOEXEC);
  return; 
}
