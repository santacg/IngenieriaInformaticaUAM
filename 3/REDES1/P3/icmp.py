...
def process_ICMP_message(us, header, data, srcIp):
    ...
    Type = data[0]
    Code = data[1]
    logging.debug(f"ICMP type: {Type}")
    logging.debug(f"ICMP code: {Code}")

    Identifier = struct.unpack("!H", data[4:6])[0]
    SequenceNumber = struct.unpack("!H", data[6:8])[0]

    if Type == ICMP_ECHO_REQUEST_TYPE:
        sendICMPMessage(data[ICMP_HEADER_LEN:], ICMP_ECHO_REPLY_TYPE,
                        Code, Identifier, SequenceNumber, srcIp)
    elif Type == ICMP_ECHO_REPLY_TYPE:
        icmp_send = srcIp + Identifier + SequenceNumber
        timeLock.acquire()
        TimeDict = icmp_send_times[icmp_send]
        timeLock.release()
        RTT = header.ts.tv_sec - TimeDict
        logging.debug(f"RTT approximate: {RTT}")
    else:
        return
...
def sendICMPMessage(data, type, code, icmp_id, icmp_seqnum, dstIP):
    ...
    header = struct.pack('!BBHHH', type, code, 0, icmp_id, icmp_seqnum)
    message = header + data
    ...
    message = struct.pack('!BBHHH', type, code, checksum,
                          icmp_id, icmp_seqnum) + data

    if type is ICMP_ECHO_REQUEST_TYPE:
        icmp_send = dstIP + icmp_id + icmp_seqnum
        timeLock.acquire()
        icmp_send_times[icmp_send] = time.time()
        timeLock.release()
