...
if __name__ == "__main__":
	...
	parser.add_argument('--icmpsize', dest='icmpsize', type=int, default=False,
                    help='Tamaño de los datos del paquete ICMP a enviar')
	...
	if args.icmpsize:
		data = (''.join(str(i % 10) for i in range(args.icmpsize))).encode()
	...
	while True:
		try:
			...
			if msg == 'q':
				...
				sendICMPMessage(data,ICMP_ECHO_REQUEST_TYPE,ICMP_ECHO_REQUEST_CODE,8,ICMP_SEQNUM,struct.unpack('!I',socket.inet_aton(args.dstIP))[0])
				ICMP_SEQNUM += 1
			elif msg == '2':
