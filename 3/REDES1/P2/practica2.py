...
if __name__ == "__main__":
	...
	while True:
		try:
			msg = input('Introduce la dirección IP a resolver o s para realizar un sondeo(q para salir y p para mostrar la caché):\n')
			if msg == 'q':
				break
			elif msg == 'p':
				printCache()
			elif msg == 's':
				ret = ARPResolution(0, True)
				if ret is not None:
					print('Direccion IP duplicada')
				else:
					print('Direccion IP no duplicada')
			else:
				try:
