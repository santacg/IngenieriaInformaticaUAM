...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if ethDest not in (macAddress, broadcastAddr):
        logging.debug('Failed to process ethernet frame')
        return 
    
    if struct.unpack('h', ethtype) in upperProtos:
        upperProtos[struct.unpack('h', ethtype)](us, header, data[14:], ethOrig)
...
def startEthernetLevel(interface:str) -> int:
    ...
    if levelInitialized == True:
        logging.error('Failed to start ethernet level')
        return -1
...
def stopEthernetLevel()->int:
    ...
    if levelInitialized == False:
        logging.error('Failed to stop ethernet level')
        return -1
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    ethData = bytearray()
    ethData.extend(header)
    ethData.extend(data)
    ethDataLen = length + 14

    if ethDataLen > ETH_FRAME_MAX:
        logging.error('Failed to send data to frame')
        return -1
    
    if ethDataLen < ETH_FRAME_MIN:
        padding = ETH_FRAME_MIN - ethDataLen
        ethData.extend(bytes([0] * padding))
        ethDataLen = ETH_FRAME_MIN

    if pcap_inject(handle, bytes(ethData), ethDataLen) == -1:
        logging.error('Failed to send data to interface')
        return -1
