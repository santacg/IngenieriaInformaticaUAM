...
MONITORS="server.resources.VotoPool.numconnused-current
...
$JEE_HOME/bin/asadmin --host $HOST --user $GFUSER --passwordfile $GFPASSFILE |
mawk -W interactive -vLOGFILE=$LOGFILE '
    BEGIN {
      numJDBCCount = numHTTPCount = numHTTPQ = -1;
    }
    /server.resources.VotoPool.numconnused-current/ {
