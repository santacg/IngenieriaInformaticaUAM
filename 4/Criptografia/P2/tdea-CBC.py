...
def generate_subkeys(key: bitarray) -> list[bitarray]: 
    ...
    c, d = permutated_key[:28], permutated_key[28:]
...
def encrypt_3des_cbc(file_in: BufferedReader, file_out: BufferedWriter):
    data = file_in.read()
    ...
    for i in range(3):
        key = util.urandom(64)
        subkeys_3des.append(generate_subkeys(key))
        print(f'KEY {i + 1}: {ba2hex(key).upper()}')

    subkeys_3des[1].reverse()
    ...
    for i in range(n_blocks):
        ...
        block_out = encrypt_block(block_in, subkeys_3des[0])
        block_out = encrypt_block(block_out, subkeys_3des[1])
        block_out = encrypt_block(block_out, subkeys_3des[2])
...
def decrypt_3des_cbc(file_in: BufferedReader, file_out: BufferedWriter, key_hex: str, iv_hex: str):
    bits_in = bitarray()
    bits_in.frombytes(file_in.read())

    subkeys_3des = []
    for i in range(3):
        key = hex2ba(key_hex[i * 16:(i + 1) * 16])
        subkeys_3des.append(generate_subkeys(key))
        print(f'KEY {i + 1}: {ba2hex(key).upper()}')

    subkeys_3des[2].reverse()
    subkeys_3des[0].reverse()
    ...
    for i in range(n_blocks):
        ...
        block_out = encrypt_block(block_in, subkeys_3des[2])
        block_out = encrypt_block(block_out, subkeys_3des[1])
        block_out = encrypt_block(block_out, subkeys_3des[0])
...
def tdea_cbc():
    ...
    Argumentos de línea de comandos:
        -C: para cifrar.
        -D: para descifrar.
        -k: claves en hexadecimal concatenadas que forman 192 bits para descifrar.
        -iv: bloque de inicialización para descifrar.
        -i: nombre del archivo de entrada.
        -o: nombre del archivo de salida.
    """
    parser = argparse.ArgumentParser(description='Cifrado TDEA en modo CBC')
    ...
    parser.add_argument('-k', type=str, required=False, help='Claves concatenadas para descifrar (escritas en hexadecimal)')
    parser.add_argument('-iv', type=str, required=False, help='Bloque de inicialización (escrito en hexadecimal)')
    ...
    if args.C and (args.k is not None or args.iv is not None):
        print("Las claves y el IV se generan aleatoriamente, no hay que introducirlos para el cifrado")
        return

    if args.C is True:
        encrypt_3des_cbc(file_in, file_out)
    else:
        ...
        decrypt_3des_cbc(file_in, file_out, args.k, args.iv) 
